# Simulation of data for phmmPen_FA method
## Alteration: add "pzerovar" value, which allows for simulation of variables that
##    have non-zero fixed effects but zero-valued random effects
sim.data.piecewise.exp.alt = function(n, ptot, pnonzero, pzerovar = 0, 
                                      nstudies, sd_raneff = 0, 
                                      B = NULL, r = 2,
  ...
  if(pzerovar > pnonzero) stop("pzerovar > pnonzero")
  ...
  if(pzerovar == 0){
    X0_tmp = cbind(rep(1,n), X0)
    if(slopes == T) Z0 = model.matrix(~drep:X0_tmp-1,  contrasts.arg=list(drep=diag(nlevels(drep))))
  }else{
    X0_tmp = cbind(rep(1,n),X0[,-c((p1-pzerovar+1):p1)])
    if(slopes == T) Z0 = model.matrix(~drep:X0_tmp-1,  contrasts.arg=list(drep=diag(nlevels(drep))))
  }
