...
# Extension of "select100_glmmPen_FA_08.R", with following changes:
#   applied to p=500 case
#   Updates to penalty sequence, which helps speed up algorithm when p=500
#   Growth ratio estimate of latent factor number r only (no input of true r)
...
dat = sim.data.piecewise.exp(n = N, ptot = p_tot, pnonzero = p, nstudies = K,
                             sd_raneff = 0, B = B, r = r,
...
lam0_seq = exp(seq(from = log(0.25), to = log(1), length.out = 10)) * lam_max
lam1_seq = exp(seq(from = log(0.10), to = log(1), length.out = 10)) * lam_max
