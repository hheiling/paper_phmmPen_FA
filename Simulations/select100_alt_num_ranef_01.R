...
library(glmmPen) # Version 1.5.4.5
library(stringr)
library(survival)
library(ncvreg) # For std() function used in sim.data.piecewise.exp.alt()
library(mvtnorm) # For use in sim.data.piecewise.exp.alt()
...
combos = expand.grid(var_start_val, conv_CD_val, cut_num_val, 
                     est_type, B_type, K_val, beta_val)
colnames(combos) = c("var_start_val", "conv_CD_val", "cut_num_val",
                    "est_type", "B_type", "K_val", "beta_val")
combos
...
# Total number of predictors of interest
p = 10
# Total number of covariates with non-zero fixed effects (10) plus intercept (total: 11)
q_f = p + 1
# Total number of covariates with non-zero valued random effects (5) plus intercept (total: 6)
q_r = 6
# Total number of covariates with non-zero valued fixed effects but zero-valued random effects (5)
q_rzero = q_f - q_r
...
# source alternative simulation function
source(str_c(header_nas, "Project3/code/sim_generation_surv_alt.R"))
...
dat = sim.data.piecewise.exp.alt(n = N, ptot = 100, pnonzero = q_f-1, pzerovar = q_rzero,
                                 nstudies = K,
                                 sd_raneff = 0, B = B, r = r,
                                 cens_type = "unif", cens_max = 5,
                                 lhaz_vals = lhaz_vals, cut_points = cut_points_input,
                                 seed = seeds[batch], imbalance = 0, beta = rep(beta,q_f-1), 
