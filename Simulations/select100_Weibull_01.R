# Perform variable selection on Piecewise Exponential data using the phmmPen_FA method 
...
combos = expand.grid(p_val, var_start_val, conv_CD_val,  
                     est_type, B_type, K_val, beta_val)
colnames(combos) = c("p_val", "var_start_val", "conv_CD_val", 
                    "est_type", "B_type", "K_val", "beta_val")
combos
...
cut_num = 8 # For input into phmmPen_FA and calculation of lambda sequence
...
dat = sim.data.weibull(n = N, ptot = 100, pnonzero = p, nstudies = K,
                       sd_raneff = 0, B = B, r = r,
                       cens_type = "unif", cens_max = 5,
                       lhaz_base = 1.0, alpha_PH = 5,
